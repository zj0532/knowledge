...
function GooFlow(bgDiv, property) {
	...
	this.$title = "默认流程";//流程图的名称
	this.$description = "流程描述";//流程图的描述（目前用来当作标题显示）
	...
	if (property.haveHead) {
		tmp = "<div class='GooFlow_head'><label>" + (property.initLabelText || "未知的流程") + "</label>";
		...
		if (property.headBtns) {
			this.$head.on("click", { inthis: this }, function (e) {
				...
				if (tar.tagName == "a") tar = tar.childNode[0];
	...
	if (this.$editable) {
		this.$workArea.on("click", { inthis: this }, function (e) {
			...
			var id = e.data.inthis.$id + "_node_" + e.data.inthis.$max;
			var json = {
				name: "node_" + e.data.inthis.$max, NodesNumber: e.data.inthis.$max,
				Display: { LocationX: X, LocationY: Y, Width: 140, Height: 30 },
				Description: "待确定类型文本",
				Action: { ActionType: e.data.inthis.$nowType }
			};
			e.data.inthis.addNode(id, json);
			e.data.inthis.$max++;
		});
		//划线时用的绑定
		...
		this.$lineOper.on("click", { inthis: this }, function (e) {
			...
			switch ($(e.target).attr("class")) {
				...
				case "b_l1":
					{
						// 取出节点，传送至设置页面进行设置
						var requestData = { Node: This.$nodeData[This.$lineData[id].From], Line: This.$lineData[id] };
						var url = "/Workflow/LineConditionWindow.aspx?r=" + Math.random();
						parent.createAndOpenModalWindowWithData("设置节点流转规则", 600, 400, url, requestData, function (r) {
							This.$nodeData[This.$lineData[id].From] = r["Node"];
						});
		...
		//格式function(id，type,json)：id是单元的唯一标识ID,type是单元的种类,有"node","line","area"三种取值,json即addNode,addLink或addArea方法的第二个传参json.
		this.onItemAdd = null;
		...
		//当修改节点描述时触发
		this.onItemDescriptionChange = null;
...
ToNodeType = {
	...
	ApproveNotAgree: "ApproveNotAgree",
	Condition: "Condition"
}

ToNodeState = {
	EffectiveBranch: "EffectiveBranch",
	ConditionNotEnougth: "ConditionNotEnougth"
}
...
GooFlow.prototype = {
	...
	addNode: function (id, json) {
		...
		if (json.Action.ActionType) {
			if (!json.Display.Width || json.Display.Width < 86) json.Display.Width = 86;
			if (!json.Display.Height || json.Display.Height < 24) json.Display.Height = 24;
			if (!json.Display.LocationX || json.Display.LocationX < 0) json.Display.LocationX = 0;
			if (!json.Display.LocationY || json.Display.LocationY < 0) json.Display.LocationY = 0;
			var hack = 0;
			if (navigator.userAgent.indexOf("8.0") != -1) hack = 2;

			var iconCls = "ico_" + json.Action.ActionType;

			this.$nodeDom[id] = $("<div class='GooFlow_item" + mark + "' id='" + id + "' style='top:" + json.Display.LocationY +
                "px;left:" + json.Display.LocationX + "px'><table cellspacing='1' style='width:" + (json.Display.Width - 2) +
                "px;height:" + (json.Display.Height - 2) + "px;'><tr><td class='ico'><b class='" + iconCls + "'></b></td><td>" + json.Display.Description +
	...
	initWorkForNode: function () {
		...
		this.$workArea.delegate(".ico", "mousedown", { inthis: this }, function (e) {
			...
			X = ev.x - t.left + This.$workArea[0].parentNode.scrollLeft;
			Y = ev.y - t.top + This.$workArea[0].parentNode.scrollTop;
			var vX = X - This.$nodeData[id].Display.LocationX, vY = Y - This.$nodeData[id].Display.LocationY;
			var isMove = false;
			document.onmousemove = function (e) {
				if (!e) e = window.event;
				var ev = mousePosition(e);
				...
				if (isMove && This.$ghost.css("display") == "none") {
					This.$ghost.css({
						display: "block",
						width: This.$nodeData[id].Display.Width - 2 + "px", height: This.$nodeData[id].Display.Height - 2 + "px",
						top: This.$nodeData[id].Display.LocationY + t.top - This.$workArea[0].parentNode.scrollTop + hack + "px",
						left: This.$nodeData[id].Display.LocationX + t.left - This.$workArea[0].parentNode.scrollLeft + hack + "px", cursor: "move"
					});
				}

				if (X < t.left - This.$workArea[0].parentNode.scrollLeft)
					X = t.left - This.$workArea[0].parentNode.scrollLeft;
				else if (X + This.$workArea[0].parentNode.scrollLeft + This.$nodeData[id].Display.Width > t.left + This.$workArea.width())
					X = t.left + This.$workArea.width() - This.$workArea[0].parentNode.scrollLeft - This.$nodeData[id].Display.Width;
				if (Y < t.top - This.$workArea[0].parentNode.scrollTop)
					Y = t.top - This.$workArea[0].parentNode.scrollTop;
				else if (Y + This.$workArea[0].parentNode.scrollTop + This.$nodeData[id].Display.Height > t.top + This.$workArea.height())
					Y = t.top + This.$workArea.height() - This.$workArea[0].parentNode.scrollTop - This.$nodeData[id].Display.Height;
				This.$ghost.css({ left: X + hack + "px", top: Y + hack + "px" });
				isMove = true;
			}
			document.onmouseup = function (e) {
		...
		this.$workArea.delegate(".GooFlow_item", "mouseup", { inthis: this }, function (e) {
			...
			var json = { FromNodeNumber: lineStart.id, ToNodeNumber: this.id, name: "" };
			if (lineStart) This.addLink(This.$id + "_line_" + This.$max, json);
			This.$max++;
		});
		...
		this.$workArea.delegate(".GooFlow_item > .span", "dblclick", { inthis: this }, function (e) {
			...
			This.$textArea.val(oldTxt).css({
				...
				left: t.left + This.$nodeData[id].Display.LocationX - This.$workArea[0].parentNode.scrollLeft - 24,
				top: t.top + This.$nodeData[id].Display.LocationY - This.$workArea[0].parentNode.scrollTop + 26
			})
				.data("id", This.$focus).focus();
			This.$workArea.parent().one("mousedown", function (e) {
				if (e.button == 2) return false;
		...
		this.$workArea.delegate(".ico + td", "dblclick", { inthis: this }, function (e) {
			...
			// 打开窗口设置节点属性
			var node = e.data.inthis.$nodeData[id];

			switch (node.Action.ActionType) {
				case ActionType.Start:
					{
						parent.$.messager.alert("提示", "你想干什么？", "info");
						break;
					}
				case ActionType.End:
					{
						parent.$.messager.alert("提示", "都结束了，你还想干什么？", "info");
						break;
					}
				case ActionType.Approve:
					{
						var requestData = node;
						var url = "/Workflow/SetupApproveNodeWindow.aspx?r=" + Math.random();
						parent.createAndOpenModalWindowWithData("审批节点设置", 450, 440, url, requestData, function (r) {
							e.data.inthis.setDescription(id, r.Description, "node");
							e.data.inthis.$nodeData[id].AutoSkip = r.AutoSkip;
							e.data.inthis.$nodeData[id].Action.Approve.Content = r.RoleIds;
						});

						/*
						var url = "/Workflow/StepApproveOptionsWindow.aspx?node_number=" + node.StepNumber +
                            "&role_ids=" + node.Action.Approve.Content;
						url = "/Workflow/SetupApproveNodeWindow.aspx";
						parent.$frameModalWindowsRequestData.push(node);
						parent.createAndOpenModalWindow("审批节点设置", 450, 440, url, function (data) {
							if (data != null) {
								e.data.inthis.setDescription(id, data.Description, "node");
								e.data.inthis.$nodeData[id].AutoSkip = data.AutoSkip;
								e.data.inthis.$nodeData[id].Action.Approve.Content = data.RoleIds;
							}
						});*/
						break;
					}
				case ActionType.CounterSign:
					{
						var url = "/Workflow/SetupCounterNodeWindow.aspx?r=" + Math.random();
						parent.createAndOpenModalWindowWithData("审批节点设置", 450, 440, url, node, function (r) {
							e.data.inthis.setDescription(id, r.Description, "node");
							e.data.inthis.$nodeData[id].AutoSkip = r.AutoSkip;
							e.data.inthis.$nodeData[id].Action.Approve.Content = r.RoleIds;
						});
            ...
            This.$textArea.val(oldTxt).css({
                ...
                left: t.left + 24 + This.$nodeData[id].DisplayLocationX - This.$workArea[0].parentNode.scrollLeft,
                top: t.top + 2 + This.$nodeData[id].DisplayLocationY - This.$workArea[0].parentNode.scrollTop
            }).data("id", This.$focus).focus();
		...
		this.$workArea.delegate(".GooFlow_item > div > div[class!=rs_close]", "mousedown", { inthis: this }, function (e) {
			...
			This.$ghost.css({
				...
				width: This.$nodeData[id].Display.Width - 2 + "px", height: This.$nodeData[id].Display.Height - 2 + "px",
				top: This.$nodeData[id].Display.LocationY + t.top - This.$workArea[0].parentNode.scrollTop + hack + "px",
				left: This.$nodeData[id].Display.LocationX + t.left - This.$workArea[0].parentNode.scrollLeft + hack + "px", cursor: cursor
			});
			var X, Y;
			X = ev.x - t.left + This.$workArea[0].parentNode.scrollLeft;
			Y = ev.y - t.top + This.$workArea[0].parentNode.scrollTop;
			var vX = (This.$nodeData[id].Display.LocationX + This.$nodeData[id].Display.Width) - X;
			var vY = (This.$nodeData[id].Display.LocationY + This.$nodeData[id].Display.Height) - Y;
			var isMove = false;
			This.$ghost.css("cursor", cursor);
			document.onmousemove = function (e) {
				if (!e) e = window.event;
				var ev = mousePosition(e);
				X = ev.x - t.left + This.$workArea[0].parentNode.scrollLeft - This.$nodeData[id].Display.LocationX + vX;
				Y = ev.y - t.top + This.$workArea[0].parentNode.scrollTop - This.$nodeData[id].Display.LocationY + vY;
				if (X < 86) X = 86;
				if (Y < 24) Y = 24;
				isMove = true;
				switch (cursor) {
	...
	focusItem: function (id, bool) {
		...
		if (jq.prop("tagName") == "DIV") {
			...
			var x, y, from, to;
			if (GooFlow.prototype.useSVG != "") {
				from = jq[0].getAttribute("From").split(",");
				to = jq[0].getAttribute("To").split(",");
			} else {
	...
	moveNode: function (id, left, top) {
		...
		if (this.$undoStack) {
			var paras = [id, this.$nodeData[id].Display.LocationX, this.$nodeData[id].Display.LocationY];
			this.pushOper("moveNode", paras);
		}
		if (left < 0) left = 0;
		if (top < 0) top = 0;
		$("#" + id).css({ left: left + "px", top: top + "px" });
		this.$nodeData[id].Display.LocationX = left;
		this.$nodeData[id].Display.LocationY = top;
		//重画转换线
	...
	setDescription: function (id, text, type) {
		if (type == "node") {//如果是结点
			if (!this.$nodeData[id]) return;
			if (this.$nodeData[id].Display.Description == text) return;
			if (this.onItemDescriptionChange != null && !this.onItemDescriptionChange(id, text, "node")) return;
			this.$nodeData[id].Display.Description = text;
			...
			if (this.$nodeDom[id]) {
				this.$nodeDom[id].find("td:eq(1)").text(text);
				var hack = 0;
				if (navigator.userAgent.indexOf("8.0") != -1) hack = 2;
				...
				this.$nodeData[id].Display.Width = width;
				this.$nodeData[id].Display.Height = height;
			}
	...
	resizeNode: function (id, width, height) {
		...
		if (this.$undoStack) {
			var paras = [id, this.$nodeData[id].Display.Width, this.$nodeData[id].Display.Height];
	...
	setTitle: function (text) {
		...
		// 这里应该显示流程标题，目前先使用描述当作标题
		//if (this.$head) this.$head.children("label").attr("title", text).text(text);
		if (this.$head) this.$head.children("label").text(text);
	},
	...
	loadData: function (data) {
		...
		var t = this.$editable;
		this.$editable = false;
		if (data.Description) this.setTitle(data.Description);

		// 初始化节点
		for (var i = 0; i < data.Nodes.length; i++) {
			var nodeId = data.Nodes[i].NodeNumber;
			this.addNode(nodeId, data.Nodes[i]);
		}

		// 初始化连接线
		for (var i = 0; i < data.Nodes.length; i++) {
			for (var j = 0; j < data.Nodes[i].NextNodes.length; j++) {
				var linkId = "FlowNodeLink_" + i + "_" + j;
				var linkJson = {
					FromNodeNumber: data.Nodes[i].NodeNumber,
					ToNodeNumber: data.Nodes[i].NextNodes[j].ToNodeNumber,
					NodeType: data.Nodes[i].NextNodes[j].Type
				};
				this.addLink(linkId, linkJson);
			}
		}
        ...
        for (var i = 0; i < data.Links.length; i++) {
            //this.addNode(data.Links[i].StepNumber, data.Links[i]);
        	var linkId = "AutoLinkId_" + i;
        	//data.Links[i].From = "AutoNodeId_" + data.Links[i].From;
        	//data.Links[i].To = "AutoNodeId_" + data.Links[i].To;
        	this.addLink(linkId, data.Links[i]);
        }*/
	...
	exportData: function () {
		...
		for (var node in this.$nodeData) {
			nodes.push(this.$nodeData[node]);
		}

		for (var link in this.$lineData) {
			links.push(this.$lineData[link]);
		}

		var ret = {
			Name: this.$title, FlowCode: "", Title: this.$title, Description: this.$description,
			Nodes: nodes, Links: links, Areas: this.$areaData, initNum: this.$max
		};
	...
	calcStartEnd: function (n1, n2) {
		var X_1, Y_1, X_2, Y_2;
		//X判断：
		var x11 = n1.Display.LocationX, x12 = n1.Display.LocationX + n1.Display.Width, x21 = n2.Display.LocationX, x22 = n2.Display.LocationX + n2.Display.Width;
		//结点2在结点1左边
		if (x11 >= x22) {
			X_1 = x11; X_2 = x22;
		}
		...
		//Y判断：
		var y11 = n1.Display.LocationY,
            y12 = n1.Display.LocationY + n1.Display.Height,
            y21 = n2.Display.LocationY,
            y22 = n2.Display.LocationY + n2.Display.Height;
	...
	calcPolyPoints: function (n1, n2, type, M) {
		//开始/结束两个结点的中心
		var SP = { x: n1.Display.LocationX + n1.Display.Width / 2, y: n1.Display.LocationY + n1.Display.Height / 2 };
		var EP = { x: n2.Display.LocationX + n2.Display.Width / 2, y: n2.Display.LocationY + n2.Display.Height / 2 };
		var sp = [], m1 = [], m2 = [], ep = [];
		...
		if (type == "lr") {
			...
			m2 = [M, EP.y];
			//再具体分析修改开始点和中点1
			if (m1[0] > n1.Display.LocationX && m1[0] < n1.Display.LocationX + n1.Display.Width) {
				m1[1] = (SP.y > EP.y ? n1.Display.LocationY : n1.Display.LocationY + n1.Display.Height);
				sp[0] = m1[0]; sp[1] = m1[1];
			}
			else {
				sp[0] = (m1[0] < n1.Display.LocationX ? n1.Display.LocationX : n1.Display.LocationX + n1.Display.Width)
			}
			//再具体分析中点2和结束点
			if (m2[0] > n2.Display.LocationX && m2[0] < n2.Display.LocationX + n2.Display.Width) {
				m2[1] = (SP.y > EP.y ? n2.Display.LocationY + n2.Display.Height : n2.Display.LocationY);
				ep[0] = m2[0]; ep[1] = m2[1];
			}
			else {
				ep[0] = (m2[0] < n2.Display.LocationX ? n2.Display.LocationX : n2.Display.LocationX + n2.Display.Width)
			}
		}
			...
			m2 = [EP.x, M];
			//再具体分析修改开始点和中点1
			if (m1[1] > n1.Display.LocationY && m1[1] < n1.Display.LocationY + n1.Display.Height) {
				m1[0] = (SP.x > EP.x ? n1.Display.LocationX : n1.Display.LocationX + n1.Display.Width);
				sp[0] = m1[0]; sp[1] = m1[1];
			}
			else {
				sp[1] = (m1[1] < n1.Display.LocationY ? n1.Display.LocationY : n1.Display.LocationY + n1.Display.Height)
			}
			//再具体分析中点2和结束点
			if (m2[1] > n2.Display.LocationY && m2[1] < n2.Display.LocationY + n2.Display.Height) {
				m2[0] = (SP.x > EP.x ? n2.Display.LocationX + n2.Display.Width : n2.Display.LocationX);
				ep[0] = m2[0]; ep[1] = m2[1];
			}
			else {
				ep[1] = (m2[1] < n2.Display.LocationY ? n2.Display.LocationY : n2.Display.LocationY + n2.Display.Height);
			}
		}
	...
	getMValue: function (n1, n2, mType) {
		if (mType == "lr") {
			return (n1.Display.LocationX + n1.Display.Width / 2 + n2.Display.LocationX + n2.Display.Width / 2) / 2;
		}
		else if (mType == "tb") {
			return (n1.Display.LocationY + n1.Display.Height / 2 + n2.Display.LocationY + n2.Display.Height / 2) / 2;
		}
	},

	//增加一条线
	addLink: function (id, json) {

		// 执行事件
		if (this.onItemAdd != null && !this.onItemAdd(id, "link", json)) return;

		if (this.$undoStack && this.$editable) {
			this.pushOper("delLink", [id]);
		}

		if (json.FromNodeNumber == json.ToNodeNumber) return;

		// 避免两个节点间不能有一条以上同向接连线
		for (var k in this.$lineData) {
			if ((json.FromNodeNumber == this.$lineData[k].FromNodeNumber && json.ToNodeNumber == this.$lineData[k].ToNodeNumber)) return;
		}

		var fromNodeId = json.FromNodeNumber;
		var toNodeId = json.ToNodeNumber;

		// 获取开始/结束结点的数据
		var n1 = this.$nodeData[fromNodeId];
		var n2 = this.$nodeData[toNodeId];
		if (!n1 || !n2) return;
		...
		this.$lineData[id].From = fromNodeId;
		this.$lineData[id].To = toNodeId;
		this.$lineData[id].Name = json.Name;

		/*
		// 更新对应节点的连接线信息
		var fromNode = this.$nodeData[fromNodeId];
		if (fromNode && fromNode.NextNodes) {
			var hasToNode = false;
			// 循环找到所有跳转节点，检查是否已存在
			for (var toNode in fromNode.NextNodes) {
				if (toNode.ToNodeNumber == toNodeId) {
					hasToNode = true;
					break;
				}
			}
			...
			if (!hasToNode) {
				var toNode = { Type: ToNodeType.Normal, ToNodeNumber: toNodeId, ToNodeState: ToNodeState.EffectiveBranch };
		...
		if (json.mark) this.$lineData[id].marked = json.mark ? true : false;
		...
		if (GooFlow.prototype.useSVG == "") {
			...
			// childNodes 一个就2个，
			this.$lineDom[id].childNodes[1].textContent = json.Name;
		}
		++this.$lineCount;
		if (this.$editable) {
			this.$lineData[id].alt = true;
	...
	resetLines: function (nodeId, node) {
		for (var i in this.$lineData) {
			var other = null;//获取结束/开始结点的数据
			var res;

			//找结束点
			if (this.$lineData[i].From == nodeId) {
	...
	delLine: function (id) {
		...
		if (this.$undoStack) {
			...
			this.pushOper("addLink", paras);
		}
		this.$draw.removeChild(this.$lineDom[id]);

		// 从集合中删除已有跳转
		var lineData = this.$lineData[id];
		var nodeData = this.$nodeData[lineData.From];
		//console.info(nodeData.NextNodes.length);
		for (var i = 0; i < nodeData.NextNodes.length; i++) {
			//console.info("i");
			//console.info(lineData.To);
			if (lineData.To == nodeData.NextNodes[i]["ToNodeNumber"]) {
				//nodeData.NextNodes.splice(i);
			}
		}
